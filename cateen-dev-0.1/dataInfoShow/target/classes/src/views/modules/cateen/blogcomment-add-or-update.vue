<template>
  <el-dialog
    ...
    <el-form-item label="评论所属的blog id" prop="blogid">
      <el-input v-model="dataForm.blogid" placeholder="评论所属的blog id"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          blogid: [
            { required: true, message: '评论所属的blog id不能为空', trigger: 'blur' }
          ],
          parentid: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/cateen/blogcomment/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.content = data.blogcomment.content
                this.dataForm.blogid = data.blogcomment.blogid
                this.dataForm.parentid = data.blogcomment.parentid
                this.dataForm.frompublisherid = data.blogcomment.frompublisherid
                this.dataForm.topublisherid = data.blogcomment.topublisherid
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/cateen/blogcomment/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
