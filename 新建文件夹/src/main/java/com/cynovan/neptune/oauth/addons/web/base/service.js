define([], function (SparkMD5) {
    ...
    app.service('dialog', ['template', '$compile', '$rootScope', '$controller', '$window',
        function (template, $compile, $rootScope, $controller, $window) {
    ...
    app.service('websocket', ['session', 'util', function (session, util) {
        var socketMap = {}, stompClientMap = {}, subscribeHeartBeatMap = {};
        ...
        var service = {
            sub: function (options) {
                ...
                stompClient.connect({user_id: session.user.id}, function (frame) {
                    stompClient.subscribe(topic, _.throttle(function (result) {
                        if (options.once) {
                            service.unsub(topic);
                        } else {
                            if (options.heartbeat) {
                                var intervalEvent = _.get(subscribeHeartBeatMap, topic, '');
                                if (!intervalEvent) {
                                    ...
                                    _.set(subscribeHeartBeatMap, topic, intervalEvent);
                                }
                            }
                        }
                        if (_.isFunction(options.onmessage)) {
                            if (result.body) {
                                ...
                                options.onmessage.call(null, data);
                            }
                        }
                    }, options.throttle));
                    if (options.scope) {
            ...
            unsub: function (topic) {
                ...
                if (intervalEvent) {
                    ...
                    delete subscribeHeartBeatMap[topic];
                }
    ...
    app.service('FileUploadService', ['dialog', function (dialog) {
        var service = {
