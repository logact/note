...
public class EntityLib {
    ...
    public static Bson appendCompanyID(Bson document) {
        if(document == null){
            document = DocumentLib.newDoc();
        }
        Document filter = DocumentLib.bsonToDocument(document);
        if (filter != null) {
            if (filter.containsKey(QUserInfo.company_id) == false) {
                String companyId = UserUtils.getCompanyId();
                if (StringLib.isEmpty(companyId)) {
                    companyId = "no company id";
                }
                filter.put(QUserInfo.company_id, companyId);
            }
        }
        return filter;
    }
