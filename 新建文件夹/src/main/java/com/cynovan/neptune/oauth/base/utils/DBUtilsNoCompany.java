...
public class DBUtilsNoCompany {
    ...
    public static void createIndex(String collectionName, Document index, boolean background) {
        DBUtilsNoCompany.getCollection(collectionName).createIndex(index, new IndexOptions().background(background));
    }
    ...
    public static long count(String collectionName, Bson filter) {
        ...
        MongoCollection<Document> mongoCollection = DBUtilsNoCompany.getCollection(collectionName);
    ...
    public static void save(String collectionName, Document document) {
        ...
        if (objectId != null) {
            ...
            UpdateResult updateResult = DBUtilsNoCompany.updateOne(collectionName, DocumentLib.newDoc("_id", objectId), DocumentLib.new$Set(document));
    ...
    public static DeleteResult deleteMany(String collectionName) {
        return deleteMany(collectionName, new Document());
    }
    ...
    private static Bson processIdFilter(Bson bson) {
        if (bson != null) {
            Document doc = bsonToDocument(bson);
            Object idValue = null;
            ...
            if (idValue != null) {
                ...
                if (idValue instanceof ObjectId) {
                    ...
                    if (ObjectId.isValid(StringLib.toString(idValue))) {
                        objectIdValue = new ObjectId(StringLib.toString(idValue));
                    }
                } else {
                    objectIdValue = idValue;
                }
